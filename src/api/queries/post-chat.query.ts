...
export const queryPostChat = (model: OllamaSupportedModel, conversation: OllamaConversation, tools?: OllamaToolRequest[]): Promise<OllamaChatResponse> => {
    try {
    ...
    return fetch(
        `${OllamaAPIHandler.BASE_URL}/api/chat`,
        {
            method: "POST",
            body: JSON.stringify({
                model: model,
